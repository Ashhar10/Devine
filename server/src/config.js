...
export const config = {
  ...
  corsAllowedOrigins:
    (process.env.ALLOW_ORIGINS || process.env.ALLOW_ORIGIN || 'https://ashhar10.github.io,http://localhost:8000,http://localhost:5500,http://127.0.0.1:5500,https://devine-eu5yhxjqp-ashhar10s-projects.vercel.app')
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean),
};